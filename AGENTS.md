...
> 生效优先级：AGENTS.md > docs/ops/RUNBOOK.md > docs/testing/TEST_PLAN.md > docs/testing/TEST_REPORT.md > docs/adr/* > DEVELOPMENT_LOG.md
...
1. Read-first Checkpoint（任务开始）
2. Plan（≤5 条）
3. Executed Commands（可复制）
4. Diff Summary（文件列表 + 关键片段）
5. Tests（命令 + 结果）
6. DoD Checklist（逐条通过）
7. Push Gate：若准备 push，必须询问用户许可
8. - 大型文档正文默认不在聊天中全文粘贴；必须直接写入仓库对应文件。
   - 聊天中只输出：文件路径、git diff --name-only、关键 diff 片段（≤200行）、最小验证命令与结果摘要、commit hash。
   - 仅当用户明确要求“贴全文”，才允许在聊天中输出全文。
