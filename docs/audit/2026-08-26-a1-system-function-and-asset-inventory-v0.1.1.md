...
- 版本：0.1.1
- 上一正式版本与被替代关系：替代 0.1.0（恢复提交 `67a4ce30`）；本次修订按 A2 发现收紧「前端调用」判定口径，系统规模数字与 33 条 UNUSED 结论均未变
- 日期：2026-08-26
- 状态：**Approved**（0.1.0 于 2026-08-27 获董事会「提交，继续」整体确认；0.1.1 为口径修订版，不改变已确认结论）
...
| A-01b | **前端悬空调用**（0.1.1 补录） | `api/adminConsole.ts`、`api/agent-v2.ts`、`pages/UserSettingsPage.tsx` | 15 | 无（后端不存在对应端点） | **UNUSED** | A2 反向对照发现，见 A2 报告 §5.1 |
...
> **0.1.1 口径修订（A2 发现）：** 本报告判断「路由是否有前端调用」采用**路径级子串**匹配——把路由路径去掉 `/api/v1`
> 后作为子串在前端文件中检索，**既不区分 HTTP 动词，也会被前缀相同的路由互相带命中**。A2 按 (动词, 归一化完整路径)
> 重新对接后，「有前端调用」由 **121 收敛为 94**，反向差额为 0（无新增命中）。典型假阳性：`POST /assignments`
> 命中的是 `App.tsx` 与 `routes.ts` 里的**前端路由字符串**，并非 API 调用；受影响最实质的三条是
> `POST /classes`、`POST /enrollments`、`POST /assignments`——它们实际**没有任何前端写入口**，详见 A2 报告 §2.1。
> **本修订不改变 33 条 UNUSED 的结论**（那 33 条本就路径级零命中），只收紧「有消费者」一侧的口径。
> A2 另有一项本报告完全未覆盖的发现：**前端存在 15 条指向后端不存在端点的悬空调用**——
> A1 只做「后端路由 → 找前端消费者」单向对照，该方向永远看不见此类问题，已补录为 A-01b。
...
**A1 当前状态：Approved（2026-08-27）。** 全部退出门禁达标，异源复核两轮结束、2 条 MISMATCH 均已关闭；董事会以「提交，继续」整体确认并授权进入 A2。本报告的事实与口径自此作为 A2–A6 的输入基线，后续阶段若推翻其中任一结论，须按 §5 重开点规则回到 A1 修订而非就地改写。
