...
> **但五项都尚未正式关闭。**
>
> **状态更新（2026-08-25，Phase 3 第 3b 批 `70e9c078`）：** 默认拒绝网关打断 3D 网格加载的
> 回归已修复并经浏览器实测（`/api/v1/robots/*` 资产请求 401 数为 0）。**五项仍不关闭**，
> 因为两项实测确认的缺口未动：
> (a) **对象归属**——`app/api/v1/endpoints/` 下 180 条路由中 **130 条**（含 7 条白名单公开路由）
> 在函数签名层面拿不到调用者身份，`actor.school_name` 全仓使用点为 **0**，
> 故 `AC-06` / `T-06-E` 的"越权成功 0 次、404 率 100%"目前不可能达成（影响 `AUTH-101`）；
> (b) **资产拒绝无审计**——`robots.py` 的 `_get_visible_robot_or_404` 用裸 `HTTPException(404)`，
> 未走 `raise_read_access_denied`，违反 G1 的"任何拒绝都必须留带真实资源编号的审计"（影响 `AUTH-103`）。
> 关闭判定放在 Phase 3 收口。其余 24 项仍为 `NOT_STARTED`。
