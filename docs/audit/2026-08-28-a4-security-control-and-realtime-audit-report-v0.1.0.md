...
- 版本：0.1.0
- 日期：2026-08-28
- 状态：**Ready for Board Review**（异源复核已完成，6 条 MISMATCH 已关闭、4 个独立发现已并入；等待董事会确认）
...
> **G3 的具体说明：** `POST /api/v1/classes`（后端测试 6 条）、`POST /attempts/{id}/grade`（测试 4 条）、
> `POST /training/sessions/*`（测试 12 条）这些端点的测试全部是绿的，但**没有一条测试尝试越权访问**。
> 测试绿灯与授权缺失并存，正是本门禁要防的情形，故在此显式登记。
...
| 跨租户隔离（仅 2 条实现） | A6 | 与 CLAUDE.md 的租户约束冲突 |
