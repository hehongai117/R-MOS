...
- 状态：Proposed（待用户确认旁路审批下线的前端处置后转 Accepted）
...
最后一条构成规则冲突：删除旁路审批就必须删掉一个被项目自身标记为"永不放松"的回归测试。见"待确认事项"。
...
1. **删除一个 `regression` 标记的回归测试**：`tests/regression/test_p0_bugs_2026_07.py:241` 的 `test_p0_4_approval_history_returns_records` 测的是旁路审批的 history 端点。`pytest.ini:11` 把 `regression` 定义为"断言正确行为，**永不放松**"。删除旁路审批就必须删掉它。这是规则冲突，须由用户明确批准：建议在删除的同一提交里在 `pytest.ini` 或该文件顶部记录"该 bug 所属功能已整体下线"，而不是无声删除。
2. **审批人角色下限为教师**是否符合院校实际流程；`critical` 级别是否需要两名教师或一名管理员。
3. **审计与业务共用事务**后，`log_event` 内部 `commit()` 移除会影响所有现有调用方的提交时机，需确认可接受一次性排查全部调用点。
