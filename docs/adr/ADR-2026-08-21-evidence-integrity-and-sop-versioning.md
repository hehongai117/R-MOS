...
- 状态：Proposed（待用户确认存储命名空间口径与 SOP 产品行为后转 Accepted）
...
1. **存储命名空间口径**：确认采用方案 A（泛化 `FileStorageBase` 首参）。这会改动已有契约测试。
2. **SOP 产品行为变更**：教师改已发布 SOP 需发新版本、已被引用的版本不可删除——请从教学角度确认可接受。
3. **legacy 证据的使用边界**：存量无归属、无内容复核的证据，是仅作历史展示，还是允许继续用于旧任务的报告重出。建议仅作历史展示并显式标注。
