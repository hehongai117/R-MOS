...
> **修复进度（2026-07-02）**：**P0 全部 8 个已修复** + **P1 已修复**（pytest.ini）+ **P2 已修复**（axios 1.13→1.18 + npm audit fix，**生产依赖 0 漏洞**；剩 12 个 dev 构建工具链漏洞需 `--force` 破坏性升级，暂留）。均已合并入 `quality-hardening-phase2`。**P3（DATABASE_URL 门禁）仍待处理**（需真实 Postgres）。验证：后端裸 `pytest` 692 passed/3 skipped；前端 tsc/eslint/vitest 465 passed/覆盖率门禁/build 全绿。

### P0 — 已锁定的真实功能 bug（Phase 2 特征测试锁的是错误行为，套件绿但行为错）✅ 全部已修复
| # | 端点/位置 | 现象 | 根因 |
|---|-----------|------|------|
...
### P1 — 测试基建（集成/E2E 重点）✅ 已修复
- 裸 `pytest`（无 scope）从根目录收集第三方 `data/robot-assets/*/tests/` + `scripts/backend_stress_test.py`,触发 4 个收集错误。根因：`pytest.ini` 未配 `testpaths`。**已修复**：`pytest.ini` 加 `testpaths = tests` + `norecursedirs = data scripts venv .git ...`；裸 `pytest` 现干净。

### P2 — 依赖安全 ✅ 已修复（生产依赖）
- axios 多条 high 级漏洞（SSRF / prototype pollution）。**已修复**：axios `^1.6.2`→`^1.18.1`（定向升级）+ `npm audit fix`（非破坏性）修 form-data/@remix-run/router 等 → **生产依赖 0 漏洞**。适配 axios 1.18 `CanceledError` 构造签名变化修 tsc。**剩余 12 个为 devDependencies 构建工具链漏洞**（rollup/picomatch/simple-git 等，`--omit=dev` 视图为 0），需 `npm audit fix --force` 破坏性升级，暂留避免破坏工具链。
