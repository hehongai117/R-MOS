...
> **修复进度（2026-07-02）**：**P0 全部 8 个已修复**（改代码 + 更新特征测试断言至正确行为）；**P1 已修复**（pytest.ini）。P2（axios 漏洞）、P3（DATABASE_URL 门禁）待处理。分支 `bugfix/test-plan-p0-p1`。全量后端裸 `pytest` = 692 passed / 3 skipped。
