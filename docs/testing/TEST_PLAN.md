...
2026-08-21 Phase 1 六链路审查已确认当前 E1 为 FAIL。下列项目是后续修复阶段必须新增并执行的测试，不代表当前已经实现或通过；具体证据见 `docs/audit/2026-08-21-phase1-six-chain-review-v0.1.0.md`。

| 测试组 | 最小覆盖 | 通过条件 | 当前状态 |
|---|---|---|---|
| AUTH-GATE | 非公开路由默认认证、显式公开白名单、学生/教师/学校/机器人对象归属、登录连续失败限制 | 匿名和跨对象允许 0 次；拒绝状态与真实资源审计 100% 正确；登录达到阈值后受限 | NOT_RUN |
| CTRL-GATE | 五台机器人任务绑定、现场检查、停止、幂等和并发步骤 | 跨机器人命令/快照/报告 0 条；缺状态默认阻断；停止 100% 可用；并发副作用最多 1 次 | NOT_RUN |
| EVID-GATE | 证据存在性、归属、类型、内容哈希、SOP 版本和任务完成门禁 | 伪造、跨对象、损坏、未封存证据通过 0 次；历史任务依据可回放 | NOT_RUN |
| AI-GATE | 未知动作、风险分级、教师审批、自批、审计失败和引用访问 | 未知动作默认拒绝；批准前/拒绝后/审计失败时写副作用 0 次；伪引用命中 0 条 | NOT_RUN |
| RT-GATE | WebSocket 握手认证、机器人/用户/频道隔离、心跳、切换机器人和时间格式 | 匿名连接 0 次；跨订阅消息 0 条；四个心跳周期持续健康；时间 100% 严格可解析 | NOT_RUN |
| RT-POINT-FIX | pong 入口接线、慢/半开连接发送隔离与关闭、定向参数防泄露、投递日志真实性、生成时间格式 | 端点 pong 更新状态；慢连接不阻断健康连接后续批次且有界关闭；跨用户/频道泄露 0 条；零投递不记录成功；心跳/遥测/教师事件不生成 `+00:00Z` | PASS（E1 单元/异步范围；不替代 RT-GATE） |
| DEP-GATE | 生产配置、迁移、就绪、持久化、依赖风险、发布回滚和 DR-01 至 DR-06 | 默认密钥/DEBUG/模拟回退/浮动镜像 0 次；证据重建后 100% 可读；六项 DR 全部真实 PASS | NOT_RUN |
...
> `AUTH-SCHOOLS-PII` 是 Phase 2 取证发现的新暴露面，**不属于 Phase 1 的 29 项**，单独跟踪。
...
| AUTH-GATE-13 | 匿名请求 `GET /api/v1/schools/{school_name}/teachers`（`AUTH-SCHOOLS-PII`） | 返回的 `email` 全部为脱敏形式；完整邮箱只对已认证且有权者可见 | P3-3 | NOT_RUN |
...
| 用例 | 场景 | 二元通过条件 | 批次 | 状态 |
|---|---|---|---|---|
| RT-GATE-01 | 匿名 / 过期令牌 / 无权机器人连接（令牌走首帧，5 秒未认证即断开） | 拒绝 0 次通过，且写审计；认证通过前推送遥测 0 条 | P3-6 | NOT_RUN |
...
### 自动化脚本实施说明
- 当前 Task 7-8 仅完成测试计划文档，不在本轮实现 `Playwright` 自动化脚本。
- 若进入自动化落地，优先实现：`AUTO-04`、`AUTO-05`、`AUTO-06` 与 `WS-01`（教学闭环与连接稳定性）。

### 性能基线（性能）
- 用例编号：NF-PERF-01
  - 场景：`/teaching/assignments` 首屏加载
  - 步骤：清缓存 → 打开页面 → 记录首屏时间
  - 期望结果：首屏 < 3s，表格渲染完成
  - 标签：P0
- 用例编号：NF-PERF-02
  - 场景：`GET /api/v1/assignments`
  - 步骤：`curl --noproxy 127.0.0.1,localhost http://127.0.0.1:8000/api/v1/assignments`
  - 期望结果：P95 响应 < 500ms
  - 标签：P0
- 用例编号：NF-PERF-03
  - 场景：`GET /api/v1/attempts/{attempt_id}/evidence`
  - 步骤：同一 `attempt_id` 连续调用 20 次
  - 期望结果：P95 响应 < 800ms，返回摘要完整
  - 标签：P0
- 用例编号：NF-PERF-04
  - 场景：`/monitor` WebSocket 延迟
  - 步骤：连接 `/ws/robot/status`，统计 100 条消息间隔
  - 期望结果：消息间隔均值约 200ms，抖动 < 2x
  - 标签：P1

### 稳定性（稳定性/可靠性）
- 用例编号：NF-STAB-01
  - 场景：`/monitor` 持续运行 2 小时
  - 步骤：保持连接不中断，观察内存与状态
  - 期望结果：无崩溃、无异常断开、无明显内存上涨
  - 标签：P1
- 用例编号：NF-STAB-02
  - 场景：教学尝试连续执行 20 次
  - 步骤：创建尝试 → 执行步骤 → 完成 → 重复
...
- 用例编号：NF-SEC-01
  - 场景：越权访问管理接口
  - 步骤：学生/教师访问 `/api/v1/fault-cases` 或 `/admin/faults`
  - 期望结果：返回 401/403 或重定向，前端隐藏入口
  - 标签：P0
- 用例编号：NF-SEC-02
  - 场景：输入校验（SQL/脚本注入）
  - 步骤：在故障案例新建/编辑提交注入 payload
  - 期望结果：后端拒绝或清洗输入，前端无脚本执行
  - 标签：P1
- 用例编号：NF-SEC-03
  - 场景：错误信息泄露
  - 步骤：构造非法参数触发 500
  - 期望结果：返回通用错误码，无敏感堆栈/数据库信息
  - 标签：P2
