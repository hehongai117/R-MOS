...
| M-02 认证身份与业务身份绑定 | ⚠️ **口径修正为「部分」**（见 §3） |
...
- `/ws/robot/{robot_id}/status` 的**访问授权**（当前完全不校验调用者对该 robot_id 有无权限）
  ——注意这只是授权边界，不是数据过滤，两者别混为一谈
- M-06 断点③：前端 `agent-v2.ts:193` 硬编码 `mode:'message'`，command 模式永不触发。
  但③修好后会走到④，需与决策 1 一并考虑
