...
- ~~TeacherMonitorPage 可以直接消费 `step_warning` 等班级事件~~  
  ⚠️ 已修正：当前 WebSocket 公开行为是 `telemetry` 推送 + `ping` 心跳，未暴露班级级事件。
...
⚠️ 已修正：同时补充 refresh / logout 生命周期：
  - POST /api/v1/auth/refresh
  - POST /api/v1/auth/logout
其中 refresh_token 需与本地 `rmos_refresh_token` / legacy `refresh_token` 同步维护。
...
WebSocket：
  连接 /ws/robot/status
  ✅ 已验证：服务端当前明确推送 `telemetry` 消息，并发送 `ping` 心跳；客户端需返回 `pong`
...
- 接口路径修正：4 处
- WebSocket修正：3 处
- 工具链修正：3 处（已有，删除重复建设内容）
- 其他类修正：2 处
- 后端待实现接口/事件：2 个
  - `WS /ws/robot/status` 当前未提供 `step_warning` 班级事件
  - 教师发送提示接口（路径待确认）
